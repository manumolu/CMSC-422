...
# This code comes with no warranty of any kind.  Stephen Marsland, 2008, 2014
...
def qlearn():
    ...
    #Print R,t into resultsQ.txt
    file = open('/Users/anumolubhargav/Documents/Machine Learning/Homework5/resultsDecay.txt', 'w+')
    file.write("R: {}".format(R)+ "\n"+ "\n"+ "\n")
    file.write("t: {}".format(R)+ "\n"+ "\n"+ "\n")
    
    nStates = np.shape(R)[0]
    nActions = np.shape(R)[1]
    
    Q = np.random.rand(nStates,nActions)*0.1-0.05
    Q = np.where(t==1,Q,-np.inf)
    file.write("Q: {}".format(Q)+ "\n"+ "\n"+ "\n")
    ...
    while nits < 1000:
        ...
        while s!=5:
            ...
            Q[s,a] += mu * (r + gamma*np.max(Q[sprime,aprime]) - Q[s,a])   # MODIFY
